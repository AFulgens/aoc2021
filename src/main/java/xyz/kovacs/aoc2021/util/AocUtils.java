package xyz.kovacs.aoc2021.util;
...
public class AocUtils {
	...
	public static List<String> getAllLines(Supplier<String> pathSupplier) {
		try {
			...
			String className = StringUtils.substringAfterLast(StringUtils
					                                                  .substringBeforeLast(StringUtils.substringBefore(pathSupplier.getClass().getName(), "$$Lambda$"),
					                                                                       "."),
			                                                  ".");
			path.append(className).append("/").append(pathSupplier.get());
